        ...
        this.instances[canvasId] = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    data: data,
                    backgroundColor: [
                        '#48bb78', '#38b2ac', '#4299e1', '#9f7aea', '#ed64a6', '#f6ad55'
                    ]
                }]
            },
...
const statisticsModule = {
    init: function(data) {
        ...
        data.forEach(item => {
            ...
            // Trạng thái đếm tất cả lịch
            if (statusMap[item.TrangThai] !== undefined) {
                statusMap[item.TrangThai]++;
            }

            // Chỉ thống kê doanh thu khi đã hoàn thành
        ...
        chartModule.createBarChart('chart-stat-month', 
            ['Th1', 'Th2', 'Th3', 'Th4', 'Th5', 'Th6', 'Th7', 'Th8', 'Th9', 'Th10', 'Th11', 'Th12'], 
            monthlyData, 
            `Doanh thu năm ${currentYear}`
        );

        chartModule.createLineChart('chart-stat-year', 
            Object.keys(yearlyMap).sort(), 
            Object.values(yearlyMap), 
            'Doanh thu qua các năm'
        );

        chartModule.createPieChart('chart-stat-service', 
            Object.keys(serviceMap), 
            Object.values(serviceMap)
        );

        chartModule.createPieChart('chart-stat-status', 
